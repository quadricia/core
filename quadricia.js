...
App.version =
    App.versionDetail.major +
    '.' +
    App.versionDetail.minor +
    '.' +
    App.versionDetail.patch;
...
App.import = function(req, cb, ndx) {
    if (!cb) cb = App.emptyFn;
    if (!ndx) ndx = 0;
    ...
    function urlDecode(x) {
        if (x.indexOf('@modules') > -1) {
            var mod = x.replace('@modules', 'node_modules');
            var mod_name = x.replace('@modules/', '');
            if (!App.pkg) throw '** ERROR: no modules bundler specified.';
            if (mod_name.lastIndexOf('@') > 0) {
                mod_name = mod_name.substr(0, mod_name.lastIndexOf('@'));
                mod = mod.substr(0, mod.lastIndexOf('@'));
            }
            if (App.pkg[mod_name]) {
                var newArr = App.pkg[mod_name].map((el) => mod + '/' + el);
                return newArr;
            } else {
                for (var el in App.pkg) {
                    if (el.indexOf('*') > -1) {
                        var p = mod_name.split('/')[0];
                        var m = el.substr(0, el.indexOf('*') - 1);
                        if (p == m) {
                            var newArr = App.pkg[el].map((item) => mod + '/' + item);
                            return newArr;
                        }
                    }
                }
                throw "** ERROR: Can't find module " + x;
            }
        }
        if (x.indexOf('ux') > -1) {
            var dir = App.repositories['ux.'];
            x = x.replace(/\./g, '-');
            var name = x.substr(x.lastIndexOf('-') + 1, x.length);
            return dir + x + '/' + name + '.js';
        } else {
            for (var el in App.repositories) x = x.replace(el, App.repositories[el]);
            return x;
        }
    }

    if (item.indexOf('./') > -1) rq = item;
    else {
        if (item.indexOf('://') > -1) rq = item;
        else rqs = urlDecode(req[ndx]);
    }
    if (!Array.isArray(rqs)) rqs = [rqs];
    var head = document.getElementsByTagName('head')[0];
    var listr = [];
    for (var i = 0; i < head.getElementsByTagName('script').length; i++) {
        listr.push(head.getElementsByTagName('script')[i].src);
    }
    for (var i = 0; i < head.getElementsByTagName('link').length; i++) {
        listr.push(head.getElementsByTagName('link')[i].href);
    }

    function domify(o, ndx, cb) {
        if (!o[ndx]) return cb();

        var rq = o[ndx];
        if (listr.indexOf(rq) > -1) return domify(o, ndx + 1, cb);
        if (rq.indexOf('.css') > -1) {
            ...
            listr.push(link.href);
            domify(o, ndx + 1, cb);
        } else {
            var test = rq.substr(rq.lastIndexOf('/'), rq.length);
            if (test.indexOf('.js') == -1 && rq.indexOf('unpkg') == -1)
                rq += '.min.js';
    ...
    domify(rqs, 0, function() {
        App.import(req, cb, ndx + 1);
    });
};
...
(function(global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ?
        ...
        define(factory) :
        (function() {
            var current = global.quadricia;
            var exports = factory();
            global.quadricia = exports;
            exports.noConflict = function() {
                global.quadricia = current;
                return exports;
            };
        })();
})(this, function() {
    ...
    if (!base.Promise) {
            ...
            return {
                ...
                then: function then(onFulfilled, onRejected) {
                    var promise2 = new Promise$1(function(fulfill, reject) {
                        var processResolutionHandler = function(
                            handler,
                            handlers,
                            forward
                        ) {
        ...
        Promise$1.reject = function(reason) {
            if (reason && isFunction(reason.then)) {
                return reason;
            }
            return new Promise$1(function(fulfill, reject) {
                reject(reason);
    ...
    if (hasConsole) {
        var welcomeIntro = [
            '%cQuadricia.js %c' + App.version + ' %cin debug mode',
            'color: rgb(114, 157, 52); font-weight: normal;',
            'color: rgb(114, 197, 52); font-weight: bold;',
            'color: rgb(114, 114, 114); font-weight: normal;',
        ];
        var welcomeMessage =
            "You're running Quadricia " +
            App.version +
            ' in debug mode - messages will be printed to the console to help you fix problems and optimise your application.\n\nTo disable debug mode, add this line at the start of your app:\n  App.DEBUG = false;\n\nGet help and support:\n  http://quadricia.js.org\n  http://stackoverflow.com/questions/tagged/quadriciajs\n  http://groups.google.com/forum/#!forum/quadricia-js\n  http://twitter.com/quadriciajs\n\nFound a bug? Raise an issue:\n  https://github.com/quadriciajs/quadricia/issues\n\n';
        ...
        printWarning = function(message, args) {
            ...
            if (isObjectType(args[args.length - 1])) {
                ...
                var quadricia = options ? options.quadricia : null;

                if (quadricia) {
                    // if this is an instance of a component that we know the name of, add
                    // it to the message
                    var name;
                    if (quadricia.component && (name = quadricia.component.name)) {
    ...
    var missingPlugin = function(name, type) {
        return (
            ...
            '" ' +
            type +
            ' plugin. You may need to download a plugin via http://quadricia.js.org/integrations/#' +
            type +
            's'
        );
    };

    function findInViewHierarchy(registryName, quadricia, name) {
        var instance = findInstance(registryName, quadricia, name);
        return instance ? instance[registryName][name] : null;
    }

    function findInstance(registryName, quadricia, name) {
        while (quadricia) {
            if (name in quadricia[registryName]) {
                return quadricia;
            }

            if (quadricia.isolated) {
                return null;
            }

            quadricia = quadricia.parent;
        }
    }

    function interpolate(from, to, quadricia, type) {
        if (from === to) {
            return null;
        }

        if (type) {
            var interpol = findInViewHierarchy('interpolators', quadricia, type);
            if (interpol) {
    ...
    ModelBase__proto__.addShuffleRegister = function addShuffleRegister(
        item,
        stage
    ) {
        if (stage === void 0) stage = 'early';
    ...
    ModelBase__proto__.getKeypath = function getKeypath(quadricia) {
        if (quadricia !== this.quadricia && this._link) {
            return this._link.target.getKeypath(quadricia);
        }

        if (!this.keypath) {
            var parent = this.parent && this.parent.getKeypath(quadricia);
            this.keypath = parent ?
                this.parent.getKeypath(quadricia) + '.' + escapeKey(this.key) :
                escapeKey(this.key);
        }
    ...
    ModelBase__proto__.getValueChildren = function getValueChildren(value) {
        ...
        if (isArray(value)) {
            ...
            children = keys(value).map(function(key) {
                return this$1.joinKey(escapeKey(key));
            });
        } else if (value != null) {
            children = [];
        }
        ...
        if (computed) {
            children.push.apply(
                children,
                keys(computed).map(function(k) {
                    return this$1.joinKey(k);
                })
            );
        }
    ...
    ModelBase__proto__.has = function has(key) {
        ...
        computed = this.root.quadricia && this.root.quadricia.computed;
        if (computed) {
            keys(computed).forEach(function(k) {
                if (
                    computed[k].pattern &&
                    computed[k].pattern.test(this$1.getKeypath())
                ) {
                    return true;
                }
            });
    ...
    ModelBase__proto__.notifyUpstream = function notifyUpstream(startPath) {
        ...
        while (parent) {
            if (parent.patterns) {
                parent.patterns.forEach(function(o) {
                    return o.notify(path.slice());
                });
            }
            path.unshift(parent.key);
            parent.links.forEach(function(l) {
                return l.notifiedUpstream(path, this$1.root);
            });
    ...
    ModelBase__proto__.rebind = function rebind(next, previous, safe) {
        ...
        while (i--) {
            if (this$1.deps[i].rebind) {
                this$1.deps[i].rebind(next, previous, safe);
            }
        }
        ...
        while (i--) {
            ...
            if (link.owner && link.owner._link) {
        ...
        while (i--) {
            ...
            child.rebind(
                next ? next.joinKey(child.key) : undefined,
                child._link || child,
                safe
            );
            if (this$1.dataModel) {
                this$1.addShuffleTask(function() {
                    return checkDataLink(this$1, this$1.retrieve());
                }, 'early');
            }
        }
    ...
    ModelBase__proto__.registerPatternObserver = function registerPatternObserver(
        observer
    ) {
    ...
    ModelBase__proto__.registerTwowayBinding = function registerTwowayBinding(
        binding
    ) {
    ...
    ModelBase__proto__.unregisterPatternObserver =
        function unregisterPatternObserver(observer) {
            removeFromArray(this.patterns, observer);
            this.unregister(observer);
        };

    ModelBase__proto__.unregisterTwowayBinding = function unregisterTwowayBinding(
        binding
    ) {
        removeFromArray(this.bindings, binding);
    };

    ModelBase__proto__.updateFromBindings = function updateFromBindings$1(
        cascade
    ) {
        var this$1 = this;
    ...
    function maybeBind(model, value, shouldBind) {
        if (
            ...
            if (!model.boundValue) {
                model.boundValue = bind(
                    value._r_unbound || value,
                    model.parent.quadricia
                );
            }
    ...
    function shuffle(model, newIndices, link, unsafe) {
        ...
        while (i--) {
            ...
            if (i in model.childByKey) {
                model.childByKey[i].rebind(!~idx ? undefined : model.joinKey(idx),
                    model.childByKey[i], !unsafe
                );
            }
        }
    ...
    function checkDataLink(model, value) {
        if (value !== model.dataModel) {
            if (
                value &&
                value.viewmodel &&
                value.viewmodel.isRoot &&
                model.childByKey.data
            ) {
    ...
    var LinkModel = (function(ModelBase) {
        function LinkModel(parent, owner, target, key) {
            ...
            if (owner && owner.isLink) {
                this.sourcePath = owner.sourcePath + '.' + this.key;
            }
        ...
        var LinkModel__proto__ = (LinkModel.prototype = Object.create(
            ModelBase && ModelBase.prototype
        ));
        LinkModel__proto__.constructor = LinkModel;

        LinkModel__proto__.animate = function animate(
            from,
            to,
            options,
            interpolator
        ...
        LinkModel__proto__.getKeypath = function getKeypath(quadricia) {
            if (quadricia && quadricia !== this.root.quadricia) {
                return this.target.getKeypath(quadricia);
            }

            return ModelBase.prototype.getKeypath.call(this, quadricia);
        };
        ...
        LinkModel__proto__.notifiedUpstream = function notifiedUpstream(
            startPath,
            root
        ) {
            var this$1 = this;
            ...
            this.links.forEach(function(l) {
                return l.notifiedUpstream(startPath, this$1.root);
            });
            this.deps.forEach(handleChange);
            ...
            if (next) {
                if (next === '"') {
                    ...
                    if (parser.matchString('{')) {
                        ...
                        if (!parser.matchString('}')) {
                            parser.error(
                                "Expected closing '}' after interpolated expression"
                            );
                        }
                    } else {
                        literal += '$';
    ...
    function readKeyValuePair(parser) {
        ...
        if (refKey && (parser.nextChar() === ',' || parser.nextChar() === '}')) {
            if (!spread && !name.test(key)) {
                parser.error(
                    "Expected a valid reference, but found '" + key + "' instead."
                );
            }
    ...
    function readReference(parser) {
        ...
        // bug out if it's a keyword (exception for ancestor/restricted refs - see https://github.com/quadriciajs/quadricia/issues/1497)
            ...
            switch (node.t) {
                ...
                case OBJECT_LITERAL:
                    if (node.m && hasSpread(node.m)) {
                        return (
                            'Object.assign({},' +
                            makeSpread(node.m, '{', '}', stringifyPair) +
                            ')'
                        );
                    } else {
                        return (
        ...
        function makeSpread(list, open, close, fn) {
            var out = list.reduce(
                function(a, c) {
                    if (c.p) {
                        a.str +=
                            '' + (a.open ? close + ',' : a.str.length ? ',' : '') + fn(c);
                    } else {
                        a.str +=
                            '' + (!a.str.length ? open : !a.open ? ',' + open : ',') + fn(c);
                    }
                    a.open = !c.p;
                    return a;
                }, { open: false, str: '' }
    ...
    function readQuotedAttributeValue(parser, quoteMark) {
        ...
        var token =
            readMustache(parser) || readQuotedStringToken(parser, quoteMark);
        while (token !== null) {
            tokens.push(token);
    ...
    function readAttributeOrDirective(parser) {
        ...
        if ((directive = directives[attribute.n])) {
            ...
            parser.sp();
            if (parser.nextChar() === '=') {
                attribute.f = readAttributeValue(parser);
            }
    ...
    function readInterpolator(parser, tag) {
        ...
        if (!parser.matchString(tag.close)) {
            parser.error(
                "Expected closing delimiter '" + tag.close + "' after reference"
            );
            ...
            if ((child = readClosing(parser, tag))) {
                if (expectedClose && child.r !== expectedClose) {
                    if (!block) {
                        ...
                        parser.pos = pos;
                        parser.error(
                            'Expected ' + tag.open + '/' + expectedClose + tag.close
                        );
                    }
                }
                ...
                if (child.t === ELSE) {
                    ...
                    if (hasElse) {
                        parser.error(
                            '{{then}} block must appear before any {{else}} block'
                        );
                    }
                    if (hasCatch) {
                        parser.error(
                            '{{then}} block must appear before any {{catch}} block'
                        );
                    }
                    if (hasThen) {
                    ...
                    if (hasElse) {
                        parser.error(
                            '{{catch}} block must appear before any {{else}} block'
                        );
                    }
                    if (hasCatch) {
                        parser.error(
                            'there can only be one {{catch}} block per {{#await}}'
                        );
                    }
                    mustache.t = CATCH;
                    hasCatch = true;
        ...
        while (i--) {
            ...
            if (item.f) {
                var isPreserveWhitespaceElement =
                    item.t === ELEMENT &&
                    (whiteSpaceElements[item.e.toLowerCase()] ||
                        whiteSpaceElements[item.e]);
                preserveWhitespaceInsideFragment =
                ...
                if (!anchor && !canContain(lowerCaseName, remaining)) {
                    ...
                    if (readClosing(parser, tag) || readInline(parser, tag)) {
                        ...
                        cleanup(
                            ...
                            preserveWhitespace, !preserveWhitespace, !preserveWhitespace,
        ...
        if (
            ...
            while (i < attrs.length) {
                ...
                if (a.n.indexOf('class-') === 0 && !a.f) {
                    ...
                    (styles || (styles = [])).push(
                        hyphenateCamel(a.n.slice(6)) + ': ' + a.f + ';'
                    );
                    attrs.splice(i, 1);
    ...
    function readText(parser) {
        ...
        if (
            (parser.inside && parser.inside !== 'textarea') ||
            parser.textOnlyMode
        ) {
    ...
    // See https://github.com/quadriciajs/template-spec for information
    // about the quadricia template specification
    ...
    var StandardParser = Parser.extend({
        init: function init(str, options) {
            ...
            var staticTripleDelimiters =
                options.staticTripleDelimiters ||
                shared.defaults.staticTripleDelimiters;

            this.standardDelimiters =
                options.delimiters || shared.defaults.delimiters;
            ...
            this.whiteSpaceElements = assign({},
                options.preserveWhitespace,
        ...
        postProcess: function postProcess(result, options) {
            ...
            if (options.expression) {
                ...
                cleanup(
                    parserResult.t,
                    this.stripComments,
                    this.preserveWhitespace, !this.preserveWhitespace, !this.preserveWhitespace,
                ...
                if (this.csp !== false) {
                    ...
                    if (keys(expr$1).length) {
                        parserResult.e = expr$1;
                    }
                }
    ...
    var TEMPLATE_INSTRUCTIONS =
        'Either preparse or use a quadricia runtime source that includes the parser. ';

    var COMPUTATION_INSTRUCTIONS =
        'Either include a version of quadricia that can parse or convert your computation strings to functions.';

    function throwNoParse(method, error, instructions) {
        if (!method) {
            fatal(
                'Missing quadricia.parse - cannot parse ' + error + '. ' + instructions
            );
        }
    }
    ...
    var parser = {
        fromId: function fromId(id, options) {
            if (!doc) {
                ...
                throw new Error(
                    ...
                    id +
                    ' as quadricia is not running in a browser.'
                );
            }
        ...
        getParseOptions: function getParseOptions(quadricia) {
            // Could be quadricia or a Component
            if (quadricia.defaults) {
                quadricia = quadricia.defaults;
            }

            return parseOptions.reduce(function(val, key) {
                val[key] = quadricia[key];
                return val;
            }, {});
        },
        ...
        parseFor: function parseFor(template, quadricia) {
            return this.parse(template, this.getParseOptions(quadricia));
        },
    };

    function getComputationSignature(quadricia, key, signature) {
        var getter;
        var setter;
        ...
        if (isFunction(signature)) {
            getter = bind(signature, quadricia);
            getterString = signature.toString();
            getterUseStack = true;
        }

        if (isString(signature)) {
            getter = createFunctionFromString(signature, quadricia);
            getterString = signature;
        }

        if (isObjectType(signature)) {
            if (isString(signature.get)) {
                getter = createFunctionFromString(signature.get, quadricia);
                getterString = signature.get;
            } else if (isFunction(signature.get)) {
                getter = bind(signature.get, quadricia);
            ...
            if (isFunction(signature.set)) {
                setter = bind(signature.set, quadricia);
    ...
    TransitionManager__proto__.detachNodes = function detachNodes() {
        ...
        for (var i$1 = 0; i$1 < len; i$1++) {
            this$1.children[i$1].detachNodes();
        }
        this.detachQueue = [];
    };
    ...
    var Model = (function(ModelBase) {
        ...
        var Model__proto__ = (Model.prototype = Object.create(
            ModelBase && ModelBase.prototype
        ));
        ...
        Model__proto__.adapt = function adapt() {
            ...
            var quadricia = this.root.quadricia;
            ...
            for (i = 0; i < len; i += 1) {
                ...
                if (adaptor.filter(value, keypath, quadricia)) {
        ...
        Model__proto__.applyValue = function applyValue(value, notify) {
            ...
            if (
                this.dataModel ||
                (value && value.viewmodel && value.viewmodel.isRoot)
            ) {
        ...
        Model__proto__.compute = function compute(key, computed) {
            ...
            if (registry[key]) {
                ...
                registry[key] = new shared$1.Computation(
                    this,
                    getComputationSignature(this.root.quadricia, key, computed),
                    key
                );
            }
        ...
        Model__proto__.joinKey = function joinKey(key, opts) {
            ...
            if (!child) {
                ...
                if (
                    this.isRoot &&
                    this.quadricia &&
                    (computed = this.quadricia.computed[key])
                ) {
                    child = this.compute(key, computed);
                } else if (!this.isRoot && this.root.quadricia) {
                    var registry = this.root.quadricia.computed;
                    for (var k in registry) {
                        computed = registry[k];
                        if (
                            computed.pattern &&
                            computed.pattern.test(this$1.getKeypath() + '.' + key)
                        ) {
        ...
        Model__proto__.teardown = function teardown$4() {
            ...
            if (this.computed) {
                keys(this.computed).forEach(function(k) {
                    return this$1.computed[k].teardown();
                });
            }
        };
    ...
    var SharedModel = (function(Model) {
        function SharedModel(value, name, quadricia) {
            ...
            this.root = this;
            this.adaptors = [];
            this.quadricia = quadricia;
        }

        if (Model) SharedModel.__proto__ = Model;
        var SharedModel__proto__ = (SharedModel.prototype = Object.create(
    ...
    function resolveReference(fragment, ref) {
        ...
        // ancestor references
        if (ref[0] === '~') {
            return fragment.quadricia.viewmodel.joinAll(splitKeypath(ref.slice(2)));
        }
        ...
        if (base[0] === '@') {
            ...
            // @this referring to local quadricia instance
            if (base === '@this' || base === '@') {
                return fragment.quadricia.viewmodel
                    .getquadriciaModel()
                    .joinAll(keys$$1);
                ...
                return (
                    repeater && repeater['get' + (base[1] === 'i' ? 'Index' : 'Key')]()
                );
            } else if (base === '@last') {
                ...
                var root = ref[1] === 'r' ? fragment.quadricia.root : null;
                var f = fragment;
                ...
                while (
                    ...
                    (!f.context ||
                        (f.isRoot &&
                            f.quadricia.component &&
                            (root || !f.quadricia.isolated)))
                ) {
                ...
                return new SharedModel(fragment.getContext(), 'context').joinAll(
                    keys$$1
                );
            } else if (base === '@local') {
                // @context-local data
                ...
                // @style shared model
                return fragment.quadricia.constructor._cssModel.joinAll(keys$$1);
            } else if (base === '@helpers') {
                // @helpers instance model
                return fragment.quadricia.viewmodel.getHelpers().joinAll(keys$$1);
            } else if (base === '@macro') {
        ...
        if (base && !keys$$1.length) {
            var helpers = fragment.quadricia.viewmodel.getHelpers();
            if (helpers.has(base)) {
        ...
        var shouldWarn = fragment.quadricia.warnAboutAmbiguity;
        var crossed = 0;
        var model;
        ...
        while (fragment) {
            ...
            if (model) {
                if (createMapping) {
                    model = initialFragment.quadricia.viewmodel.createLink(
                        base,
                        model,
                        base, { implicit: true }
                    );
            ...
            if (
                (fragment.componentParent ||
                    (!fragment.parent && fragment.quadricia.component)) &&
                !fragment.quadricia.isolated
            ) {
                // ascend through component boundary
                fragment = fragment.componentParent || fragment.quadricia.component.up;
                createMapping = true;
            } else {
        ...
        var instance = initialFragment.quadricia;
        if (
            instance.resolveInstanceMembers &&
            base !== 'data' &&
            base in instance
        ) {
    ...
    function up(fragment) {
        return (
            fragment &&
            ((!fragment.quadricia.isolated && fragment.componentParent) ||
                fragment.parent)
        );
    }
    ...
    function getquadriciaContext(quadricia) {
        var assigns = [],
            len = arguments.length - 1;
        ...
        var fragment =
            quadricia.fragment ||
            quadricia._fakeFragment ||
            (quadricia._fakeFragment = new FakeFragment(quadricia));
    ...
    var FakeFragment = function FakeFragment(quadricia) {
        this.quadricia = quadricia;
    };

    FakeFragment.prototype.findContext = function findContext() {
        return this.quadricia.viewmodel;
    };
    ...
    function findParentWithContext(fragment) {
        ...
        if (!frag) {
            return fragment && fragment.quadricia.fragment;
        } else {
            return frag;
        }
    }
    ...
    function gather(quadricia, keypath, base, isolated) {
        ...
        var model = base || quadricia.viewmodel;

        if (star.test(keypath)) {
            return model.findMatches(keys$$1);
        } else {
            if (model === quadricia.viewmodel) {
                ...
                if (
                    ...
                    return [
                        resolveReference(
                            quadricia.fragment || new FakeFragment(quadricia),
                            keypath
                        ),
                    ];
                } else {
    ...
    function build(quadricia, keypath, value, isolated) {
        var sets = [];
        ...
        if (isObject(keypath)) {
            var loop = function(k) {
                if (hasOwn(keypath, k)) {
                    sets.push.apply(
                        sets,
                        gather(quadricia, k, null, isolated).map(function(m) {
            ...
            sets.push.apply(
                sets,
                gather(quadricia, keypath, null, isolated).map(function(m) {
    ...
    function getComparator(option) {
        ...
        if (isString(option)) {
            return (
                comparators[option] ||
                (comparators[option] = function(thing) {
                    return thing[option];
                })
            );
        }
    ...
    function add(quadricia, keypath, d, options) {
        if (!isString(keypath) || !isNumeric(d)) {
            throw new Error('Bad arguments');
        }

        var sets = build(quadricia, keypath, d, options && options.isolated);
    ...
    function quadricia$add(keypath, d, options) {
    ...
    function animate(quadricia, model, to, options) {
        options = getOptions(options, quadricia);
        var from = model.get();
        ...
        var interpolator = interpolate(from, to, quadricia, options.interpolator);
    ...
    function quadricia$animate(keypath, to, options) {
        if (isObjectType(keypath)) {
            var keys$$1 = keys(keypath);

            throw new Error(
                'quadricia.animate(...) no longer supports objects. Instead of quadricia.animate({\n  ' +
                keys$$1
                .map(function(key) {
                ...
                .map(function(key) {
                    return (
                        "quadricia.animate('" + key + "', " + keypath[key] + ', {...});'
                    );
                })
                .join('\n') +
                '\n'
    ...
    function enqueue(quadricia, event) {
        if (quadricia.event) {
            quadricia._eventQueue.push(quadricia.event);
        }

        quadricia.event = event;
    }

    function dequeue(quadricia) {
        if (quadricia._eventQueue.length) {
            quadricia.event = quadricia._eventQueue.pop();
        } else {
            quadricia.event = null;
        }
    }
    ...
    function fireEvent(quadricia, eventName, context, args) {
        if (args === void 0) args = [];
        ...
        var eventNames = quadricia._nsSubs ?
            variants(eventName, true) :
            ['*', eventName];

        return fireEventAs(quadricia, eventNames, context, args, true);
    }

    function fireEventAs(quadricia, eventNames, context, args, initialFire) {
        if (initialFire === void 0) initialFire = false;

        var bubble = true;

        if (initialFire || quadricia._nsSubs) {
            enqueue(quadricia, context);

            var i = eventNames.length;
            while (i--) {
                if (eventNames[i] in quadricia._subs) {
                    bubble =
                        notifySubscribers(
                            quadricia,
                            quadricia._subs[eventNames[i]],
                            context,
                            args
                        ) && bubble;
                }
        ...
        if (quadricia.parent && bubble) {
            if (initialFire && quadricia.component) {
                var fullName =
                    quadricia.component.name + '.' + eventNames[eventNames.length - 1];
                eventNames = variants(fullName, false);

                if (context && !context.component) {
                    context.component = quadricia;
                }
            }

            bubble = fireEventAs(quadricia.parent, eventNames, context, args);
        }

        return bubble;
    }

    function notifySubscribers(quadricia, subscribers, context, args) {
        ...
        for (var i = 0, len = subscribers.length; i < len; i += 1) {
            if (!subscribers[i].off &&
                subscribers[i].handler.apply(quadricia, args) === false
            ) {
                stopEvent = true;
            }
        }
    ...
    Hook.prototype.fire = function fire(quadricia, arg) {
        var context = getquadriciaContext(quadricia);
        var method = this.method;

        if (quadricia[method]) {
            arg ? quadricia[method](context, arg) : quadricia[method](context);
        }
        ...
        fireEvent(
            ...
            this.event,
            context,
            arg ? [arg, quadricia] : [quadricia]
        );
    };

    function getChildQueue(queue, quadricia) {
        return queue[quadricia._guid] || (queue[quadricia._guid] = []);
    }

    function fire(hookQueue, quadricia) {
        var childQueue = getChildQueue(hookQueue.queue, quadricia);

        hookQueue.hook.fire(quadricia);
        ...
        delete hookQueue.queue[quadricia._guid];
    }
    ...
    HookQueue__proto__.begin = function begin(quadricia) {
        this.inProcess[quadricia._guid] = true;
    };

    HookQueue__proto__.end = function end(quadricia) {
        var parent = quadricia.parent;
        ...
        if (!parent || !this.inProcess[parent._guid]) {
            ...
            getChildQueue(this.queue, parent).push(quadricia);
        }

        delete this.inProcess[quadricia._guid];
    };
    ...
    function attachChild(child, options) {
        ...
        var meta = {
            instance: child,
            quadricia: this,
            name: options.name || child.constructor.name || 'quadricia',
        ...
        var rm = child.viewmodel.getquadriciaModel();
        rm.joinKey('parent', { lastLink: false }).link(
            this.viewmodel.getquadriciaModel()
        );
        rm.joinKey('root', { lastLink: false }).link(
            this.root.viewmodel.getquadriciaModel()
        );
    ...
    function quadricia$compute(path, computed) {
        var promise = runloop.start();
        ...
        if (comp) {
            ...
            var first = keys.reduce(function(a, c) {
                return a && a.childByKey[c];
            }, this.viewmodel);
            if (first) {
    ...
    function quadricia$detach() {
        if (this.isDetached) {
            return this.el;
        }

        if (this.el) {
            removeFromArray(this.el.__quadricia_instances__, this);
        }
    ...
    function quadricia$find(selector, options) {
        var this$1 = this;
        if (options === void 0) options = {};

        if (!this.rendered) {
            throw new Error(
                "Cannot call quadricia.find('" +
                selector +
    ...
    function quadricia$findAll(selector, options) {
        if (options === void 0) options = {};

        if (!this.rendered) {
            throw new Error(
                "Cannot call quadricia.findAll('" +
                selector +
                "', ...) unless instance is rendered to the DOM"
            );
        }
    ...
    function quadricia$findAllComponents(selector, options) {
    ...
    function quadricia$findComponent(selector, options) {
        var this$1 = this;
        if (options === void 0) options = {};
        ...
        if (options.remote) {
            if (!selector && this._children.length) {
    ...
    function quadricia$findContainer(selector) {
        if (this.container) {
            if (
    ...
    function quadricia$findParent(selector) {
        if (this.parent) {
    ...
    // sections when you do e.g. `quadricia.splice( 'items', 2, 2 );
        ...
        function model(mdl, args) {
            ...
            if (!isArray(array)) {
                if (isUndefined(array)) {
                    ...
                    var promise$1 = runloop.start().then(function() {
                        return result$1;
                    });
                    mdl.set(array);
                    runloop.end();
    ...
    function update$1(quadricia, model, options) {
        ...
        hooks.update.fire(quadricia, model);

        return promise;
    }

    function quadricia$update(keypath, options) {
        var opts, path;
    ...
    var ContextData = (function(Model) {
        function ContextData(options) {
            ...
            this.root = this;
            this.value = {};
            this.quadricia = options.quadricia;
            this.adaptors = [];
            this.context = options.context;
        }

        if (Model) ContextData.__proto__ = Model;
        var ContextData__proto__ = (ContextData.prototype = Object.create(
            Model && Model.prototype
        ));
    ...
    var Context = function Context(fragment, element) {
        ...
        this.quadricia = fragment.quadricia;
        this.root = this;
    };
    ...
    Context__proto__.animate = function animate$1(keypath, value, options) {
        ...
        return animate(this.quadricia, model, value, options);
    };
    ...
    Context__proto__.observe = function observe(keypath, callback, options) {
        ...
        return this.quadricia.observe(keypath, callback, options);
    };

    Context__proto__.observeOnce = function observeOnce(
        keypath,
        callback,
        options
    ) {
        ...
        options.fragment = this.fragment;
        return this.quadricia.observeOnce(keypath, callback, options);
    };
    ...
    Context__proto__.readLink = function readLink(keypath, options) {
        return this.quadricia.readLink(this.resolve(keypath), options);
    };

    Context__proto__.resolve = function resolve(path, quadricia) {
        var ref = findModel(this, path);
        var model = ref.model;
        var instance = ref.instance;
        return model ? model.getKeypath(quadricia || instance) : path;
    };
    ...
    Context__proto__.unlink = function unlink(dest) {
        var here = findModel(this, dest).model;
        var promise = runloop.start();
        if (here.owner && here.owner._link) {
            here.owner.unlink();
        }
        runloop.end();
        return promise;
    ...
    Context__proto__.update = function update(keypath, options) {
        return update$1(this.quadricia, findModel(this, keypath).model, options);
    };
    ...
    Context__proto__.getBindingPath = function getBindingPath(quadricia) {
        var ref = this.getBindingModel(this);
        var model = ref.model;
        var instance = ref.instance;
        if (model) {
            return model.getKeypath(quadricia || instance);
        }
    };
    ...
    Context__proto__.getBindingModel = function getBindingModel(ctx) {
        var el = ctx.element;
        return { model: el.binding && el.binding.model, instance: el.up.quadricia };
    };
    ...
    Context.forquadricia = getquadriciaContext;
    ...
    function findModel(ctx, path) {
        ...
        return { model: resolveReference(frag, path), instance: frag.quadricia };
    }
    ...
    function findEvent(el, name) {
        return (
            el.events &&
            el.events.find &&
            el.events.find(function(e) {
                return ~e.template.n.indexOf(name);
            })
        );
    }

    function quadricia$fire(eventName) {
        var args = [],
            len = arguments.length - 1;
        while (len-- > 0) args[len] = arguments[len + 1];
        ...
        if (args[0] instanceof Context) {
            ...
            ctx = Context.forquadricia(this, args.shift());
        } else {
            ctx = Context.forquadricia(this);
        }

        return fireEvent(this, eventName, ctx, args);
    }

    function quadricia$get(keypath, opts) {
        if (!isString(keypath)) {
    ...
    function getContext$1(node) {
        ...
        if (node) {
            if (node._quadricia) {
                return node._quadricia.proxy.getContext();
            } else if ((instances = node.__quadricia_instances__)) {
                if (instances.length === 1) {
                    return getquadriciaContext(instances[0]);
                }
            } else {
    ...
    function getContext$2(node, options) {
        if (!node) {
            return getquadriciaContext(this);
        }
    ...
    if (!svg) {
        ...
        createElement = function(type, ns, extend) {
            if (ns && ns !== html) {
                throw "This browser does not support namespaces other than http://www.w3.org/1999/xhtml. The most likely cause of this error is that you're trying to render SVG in an older browser. See http://quadricia.js.org/support/#svgs for more information";
            }
    ...
    function quadricia$insert(target, anchor) {
        if (!this.fragment.rendered) {
            // TODO create, and link to, documentation explaining this
            throw new Error(
                'The API has changed - you must call `quadricia.render(target[, anchor])` to render your quadricia instance. Once rendered you can use `quadricia.insert()`.'
            );
        }
        ...
        (
            target.__quadricia_instances__ || (target.__quadricia_instances__ = [])
        ).push(this);
    ...
    function fireInsertHook(quadricia) {
        hooks.insert.fire(quadricia);

        quadricia.findAllComponents('*').forEach(function(child) {
    ...
    function link(there, here, options) {
        var model;
        var target = (options && (options.quadricia || options.instance)) || this;
    ...
    var Observer = function Observer(quadricia, model, callback, options) {
        this.context = options.context || quadricia;
        this.callback = callback;
        this.quadricia = quadricia;
        ...
        if (isFunction(options.old)) {
            this.oldContext = create(quadricia);
    ...
    Observer__proto__.cancel = function cancel() {
        ...
        removeFromArray(this.quadricia._observers, this);
    };
    ...
    Observer__proto__.rebind = function rebind(next, previous) {
        ...
        if (next) {
            next.addShuffleTask(function() {
                return this$1.resolved(next);
            });
        }
    };
    ...
    var PatternObserver = function PatternObserver(
        quadricia,
        baseModel,
        keys$$1,
        callback,
        ...
        var baseKeypath = (this.baseKeypath = baseModel.getKeypath(quadricia));
        ...
        models.forEach(function(model) {
            this$1.newValues[model.getKeypath(this$1.quadricia)] = model.get();
        });
    ...
    PatternObserver__proto__.dispatch = function dispatch() {
        ...
        keys(newValues).forEach(function(keypath) {
            ...
            if (this$1.strict && newValue === oldValue) {
                return;
            }
        ...
        if (!this.dirty || this.changed.length) {
            ...
            if (!this.changed.length) {
                this.baseModel.findMatches(this.keys).forEach(function(model) {
                    var keypath = model.getKeypath(this$1.quadricia);
                ...
                if (this.recursive) {
                    ...
                    var ok = this.baseModel.isRoot ?
                        this.changed.map(function(keys$$1) {
                            return keys$$1.map(escapeKey).join('.');
                        }) :
                        this.changed.map(function(keys$$1) {
                            return (
                                this$1.baseKeypath + '.' + keys$$1.map(escapeKey).join('.')
                            );
                        });
    ...
    var ArrayObserver = function ArrayObserver(
        quadricia,
        model,
        callback,
        options
    ...
    function observe(keypath, callback, options) {
        ...
        keys(map).forEach(function(keypath) {
            ...
            if (keypaths.length > 1) {
                keypaths = keypaths.filter(function(k) {
                    return k;
                });
            }
        ...
        // add observers to the quadricia instance, so they can be
        // cancelled on quadricia.teardown()
    ...
    function createObserver(quadricia, keypath, callback, options) {
        var keys$$1 = splitKeypath(keypath);
        ...
        options.fragment = options.fragment || quadricia.fragment;

        var model;
        if (!options.fragment) {
            model = quadricia.viewmodel.joinKey(keys$$1[0]);
        } else {
        ...
        if (!~wildcardIndex) {
            ...
            if (options.array) {
                return new ArrayObserver(quadricia, model, callback, options);
            } else {
                return new Observer(quadricia, model, callback, options);
            }
        } else {
    ...
    function quadricia$off(eventName, callback) {
        var this$1 = this;
    ...
    function quadricia$on(eventName, callback) {
        var this$1 = this;
        ...
        var loop = function(k) {
            ...
            var caller = function() {
                ...
                if (!silent) {
                    return callback$1.apply(this, args);
                }
            };
            var entry = {
                callback: callback$1,
        ...
        return {
            cancel: function() {
                return events.forEach(function(e) {
                    return this$1.off(e[0], e[1].callback);
                });
            },
            isSilenced: function() {
                return silent;
    ...
    function quadricia$once(eventName, handler) {
    ...
    function readLink(keypath, options) {
        ...
        if (this.viewmodel.has(path[0])) {
            ...
            if (model) {
                return { quadricia: model.root.quadricia, keypath: model.getKeypath() };
            }
        }
    }

    var PREFIX = '/* quadricia.js component styles */';
    ...
    function getCSS(cssIds) {
        ...
        var filteredStyleDefinitions = cssIds ?
            styleDefinitions.filter(function(style) {
    ...
    function style() {
        ...
        if (doc && !styleElement) {
            ...
            styleElement.setAttribute('data-quadricia-css', '');
    ...
    var dataRvcGuidPattern = /\[data-quadricia-css~="\{[a-z0-9-]+\}"]/g;
    ...
    function transformCss(css, id) {
        var dataAttr = '[data-quadricia-css~="{' + id + '}"]';
    ...
    var CSSModel = (function(SharedModel) {
        ...
        var CSSModel__proto__ = (CSSModel.prototype = Object.create(
            SharedModel && SharedModel.prototype
        ));
        CSSModel__proto__.constructor = CSSModel;

        CSSModel__proto__.downstreamChanged = function downstreamChanged(
            path,
            depth
        ) {
            if (this.locked) {
    ...
    function initCSS(options, target, proto) {
        ...
        var def = {
            transform: 'noCSSTransform' in options ?
                !options.noCSSTransform :
                !options.noCssTransform,
        };
    ...
    function validate(data) {
        ...
        if (data && data.constructor !== Object) {
            if (isFunction(data)) {
                // TODO do we need to support this in the new quadricia() case?
    ...
    var dataConfigurator = {
                ...
                for (key in options.data) {
                    ...
                    if (value && isObjectType(value)) {
                        if (isObject(value) || isArray(value)) {
                            warnIfDebug(
                                'Passing a `data` option with object and array properties to quadricia.extend() is discouraged, as mutating them is likely to cause bugs. Consider using a data function instead:\n\n  // this...\n  data: function () {\n    return {\n      myObject: {}\n    };\n  })\n\n  // instead of this:\n  data: {\n    myObject: {}\n  }'
                            );
                        }
                    }
                }
        ...
        init: function(Parent, quadricia, options) {
            var result = combine$1(Parent.prototype.data, options.data);

            if (isFunction(result)) {
                result = result.call(quadricia);
            }

            // bind functions to the quadricia instance at the top level,
            // unless it's a non-POJO (in which case alarm bells should ring)
            if (result && result.constructor === Object) {
                for (var prop in result) {
                    if (isFunction(result[prop])) {
                        var value = result[prop];
                        result[prop] = bind(value, quadricia);
        ...
        reset: function reset(quadricia) {
            var result = this.init(
                quadricia.constructor,
                quadricia,
                quadricia.viewmodel
            );
            quadricia.viewmodel.root.set(result);
            return true;
        },
    };
    ...
    function combine$1(parentValue, childValue) {
        ...
        // the default data object on quadricia or a component.
        // then quadricia.set() ends up setting on the prototype!
    ...
    var templateConfigurator = {
        ...
        init: function init(Parent, quadricia, options) {
            // TODO because of prototypal inheritance, we might just be able to use
            // quadricia.template, and not bother passing through the Parent object.
            ...
            if (isFunction(template)) {
                var fn = template;
                template = getDynamicTemplate(quadricia, fn);

                quadricia._config.template = {
                    fn: fn,
                    result: template,
                };
            }

            template = parseTemplate(template, quadricia);

            // TODO the naming of this is confusing - quadricia.template refers to [...],
            // but Component.prototype.template refers to {v:1,t:[],p:[]}...
            // it's unnecessary, because the developer never needs to access
            // quadricia.template
            quadricia.template = template.t;

            if (template.p) {
                extendPartials(quadricia.partials, template.p);
            }
        },

        reset: function reset(quadricia) {
            var result = resetValue(quadricia);

            if (result) {
                var parsed = parseTemplate(result, quadricia);

                quadricia.template = parsed.t;
                extendPartials(quadricia.partials, parsed.p, true);
    ...
    function resetValue(quadricia) {
        var initial = quadricia._config.template;
        ...
        var result = getDynamicTemplate(quadricia, initial.fn);
    ...
    function getDynamicTemplate(quadricia, fn) {
        return fn.call(quadricia, {
            fromId: parser.fromId,
            isParsed: parser.isParsed,
            parse: function parse(template, options) {
                if (options === void 0) options = parser.getParseOptions(quadricia);
    ...
    function parseTemplate(template, quadricia) {
        if (isString(template)) {
            // parse will validate and add expression functions
            template = parseAsString(template, quadricia);
        } else {
    ...
    function parseAsString(template, quadricia) {
        // ID of an element containing the template?
        if (template[0] === '#') {
            template = parser.fromId(template);
        }

        return parser.parseFor(template, quadricia);
    }
    ...
    function validate$1(template) {
        ...
        if (template == undefined) {
            ...
            throw new Error(
                ...
                ', got ' +
                template.v +
                ') Please ensure you are using the latest version of quadricia.js in your build process as well as in your app'
            );
        }
    }
    ...
    Registry__proto__.reset = function reset(quadricia) {
        var registry = quadricia[this.name];
        var changed = false;
    ...
    function getMessage(deprecated, correct, isError) {
        return (
            ...
            '.' +
            (isError ?
                ' You cannot specify both options, please use options.' +
                correct +
                '.' :
                '')
        );
    ...
    var config = {
        extend: function(Parent, proto, options, Child) {
            return configure('extend', Parent, proto, options, Child);
        },
        init: function(Parent, quadricia, options) {
            return configure('init', Parent, quadricia, options);
        },
        reset: function(quadricia) {
            return order
                .filter(function(c) {
                    return c.reset && c.reset(quadricia);
                })
                .map(function(c) {
                    return c.name;
                });
    ...
    var isStandardKey = makeObj(
        defaultKeys.filter(function(key) {
            return !custom[key];
        })
    );
        ...
        for (var key in options) {
            if (hasOwn(isStandardKey, key)) {
                ...
                if (key !== 'el' && isFunction(value)) {
                    warnIfDebug(
                        key +
                        ' is a quadricia option that does not expect a function and will be ignored',
            ...
            if (!isBlacklisted[key] && hasOwn(options, key)) {
                ...
                if (isFunction(member)) {
                    if (
                        (key in proto$9 ||
                            (key.slice(0, 2) === 'on' &&
                                key.slice(2) in hooks &&
                                key in target)) &&
                        !_super.test(member.toString())
                    ) {
                        warnIfDebug(
                            "Overriding quadricia prototype function '" +
                            key +
                            "' without calling the '" +
                            _super +
    ...
    var Item = function Item(options) {
        this.up = options.up;
        this.quadricia = options.up.quadricia;
    ...
    var ContainerItem = (function(Item) {
        ...
        var ContainerItem__proto__ = (ContainerItem.prototype = Object.create(
            Item && Item.prototype
        ));
        ...
        ContainerItem__proto__.findAllComponents = function findAllComponents(
            name,
            options
        ) {
            if (this.fragment) {
    ...
    function getUpdateDelegate(attribute) {
        ...
        if (
            name === 'class' &&
            (!node.namespaceURI || node.namespaceURI === html)
        ) {
    ...
    function updateMultipleSelectValue(reset) {
        ...
        if (reset) {
            ...
            while (i--) {
                ...
                var optionValue = option._quadricia ?
                    option._quadricia.value :
                    option.value; // options inserted via a triple don't have _quadricia
    ...
    function updateSelectValue(reset) {
        ...
        if (!this.locked) {
            ...
            this.node._quadricia.value = value;
    ...
    function updateRadioValue(reset) {
        ...
        node.value = this.node._quadricia.value = value;
    ...
    function updateValue(reset) {
        if (!this.locked) {
            if (reset) {
                ...
                this.node.value = this.node._quadricia.value = null;
            } else {
                var value = this.getValue();

                this.node.value = this.node._quadricia.value = value;
    ...
    var ConditionalAttribute = (function(Item) {
        ...
        var ConditionalAttribute__proto__ = (ConditionalAttribute.prototype =
            Object.create(Item && Item.prototype));
    ...
    var Attribute = (function(Item) {
        function Attribute(options) {
            ...
            this.quadricia = this.up.quadricia;
            ...
            if (this.interpolator) {
                this.interpolator.owner = this;
            }
        }

        if (Item) Attribute.__proto__ = Item;
        var Attribute__proto__ = (Attribute.prototype = Object.create(
            Item && Item.prototype
        ));
        ...
        Attribute__proto__.render = function render() {
            ...
            if (!node.namespaceURI || node.namespaceURI === namespaces.html) {
                ...
                if (this.propertyName === 'value') {
                    node._quadricia.value = this.value;
                }
            }
    ...
    var BindingFlag = (function(Item) {
        ...
        var BindingFlag__proto__ = (BindingFlag.prototype = Object.create(
            Item && Item.prototype
        ));
    ...
    function quadricia$teardown() {
        var this$1 = this;

        if (this.torndown) {
            warnIfDebug(
                'quadricia.teardown() was called on a quadricia instance that was already torn down'
            );
            return Promise.resolve();
        }
        ...
        return teardown$1(this, function() {
            return this$1.fragment.rendered ? this$1.unrender() : Promise.resolve();
        });
    }
    ...
    function teardown$1(instance, getPromise) {
        ...
        if (instance.el && instance.el.__quadricia_instances__) {
            removeFromArray(instance.el.__quadricia_instances__, instance);
        }
    ...
    var quadriciaModel = (function(SharedModel) {
        function quadriciaModel(quadricia) {
            SharedModel.call(this, quadricia, '@this');
            this.quadricia = quadricia;
        }

        if (SharedModel) quadriciaModel.__proto__ = SharedModel;
        var quadriciaModel__proto__ = (quadriciaModel.prototype = Object.create(
            SharedModel && SharedModel.prototype
        ));
        quadriciaModel__proto__.constructor = quadriciaModel;

        quadriciaModel__proto__.joinKey = function joinKey(key) {
            ...
            if ((key === 'root' || key === 'parent') && !model.isLink) {
                ...
                return this.quadricia.constructor._cssModel;
            }

            return model;
        };

        return quadriciaModel;
    })(SharedModel);
    ...
    function initLink(model, key) {
        model.applyValue = function(value) {
            ...
            if (value && value.viewmodel) {
                this.link(value.viewmodel.getquadriciaModel(), key);
        ...
        model.applyValue(model.parent.quadricia[key], key);
    ...
    var specialModels = {
        ...
        '@style': function _style(root) {
            return root.getquadriciaModel().joinKey('cssData');
        },
    ...
    var RootModel = (function(Model) {
        function RootModel(options) {
            ...
            this.isRoot = true;
            this.root = this;
            this.quadricia = options.quadricia; // TODO sever this link
        ...
        var RootModel__proto__ = (RootModel.prototype = Object.create(
            Model && Model.prototype
        ));
        ...
        RootModel__proto__.createLink = function createLink(
            keypath,
            target,
            targetPath,
            options
        ...
        RootModel__proto__.getHelpers = function getHelpers() {
            if (!this.helpers) {
                this.helpers = new SharedModel(
                    this.quadricia.helpers,
                    'helpers',
                    this.quadricia
                );
            }
        ...
        RootModel__proto__.getquadriciaModel = function getquadriciaModel() {
            return (
                this.quadriciaModel ||
                (this.quadriciaModel = new quadriciaModel(this.quadricia))
            );
        };
        ...
        RootModel__proto__.has = function has(key) {
            ...
            if (Model.prototype.has.call(this, key)) {
                ...
                if (
                    this.childByKey[unescapedKey] &&
                    this.childByKey[unescapedKey]._link
                ) {
                    return true;
                }
            }
        ...
        RootModel__proto__.teardown = function teardown() {
            ...
            this.quadriciaModel && this.quadriciaModel.teardown();
        };
    ...
    function attachImplicits(model, fragment) {
        ...
        for (var k in model.childByKey) {
            if (model.value) {
                ...
                } else if (!model.childByKey[k]._link ||
    ...
    function construct(quadricia, options) {
        if (App.DEBUG) welcome();

        initialiseProperties(quadricia);
        handleAttributes(quadricia);

        // set up event subscribers
        subscribe(quadricia, options, 'on');

        // if there's not a delegation setting, inherit from parent if it's not default
        if (!hasOwn(options, 'delegate') &&
            quadricia.parent &&
            quadricia.parent.delegate !== quadricia.delegate
        ) {
            quadricia.delegate = false;
        }
        ...
        if (isArray(options.use)) {
            quadricia.use.apply(
                quadricia,
                options.use.filter(function(p) {
                    return p.construct;
                })
            );
        }

        hooks.construct.fire(quadricia, options);
        if (options.onconstruct) {
            options.onconstruct.call(
                quadricia,
                getquadriciaContext(quadricia),
                options
            );
        }
        ...
        while (i--) {
            ...
            quadricia[name] = assign(
                create(quadricia.constructor[name] || null),
                options[name]
            );
        }
        ...
        while (i--) {
            ...
            quadricia[name$1] = assign(
                create(quadricia.constructor.prototype[name$1]),
                options[name$1]
            );
        }

        if (quadricia._attributePartial) {
            quadricia.partials['extra-attributes'] = quadricia._attributePartial;
            delete quadricia._attributePartial;
        }

        // Create a viewmodel
        var viewmodel = new RootModel({
            adapt: getAdaptors(quadricia, quadricia.adapt, options),
            data: dataConfigurator.init(quadricia.constructor, quadricia, options),
            quadricia: quadricia,
        });

        // once resolved, share the adaptors array between the root model and instance
        quadricia.adapt = viewmodel.adaptors;

        quadricia.viewmodel = viewmodel;

        for (var k in quadricia.computed) {
            compute.call(quadricia, k, quadricia.computed[k]);
        }
    }

    function getAdaptors(quadricia, protoAdapt, options) {
        protoAdapt = protoAdapt.map(lookup);
        var adapt = ensureArray(options.adapt).map(lookup);

        var srcs = [protoAdapt, adapt];
        if (quadricia.parent && !quadricia.isolated) {
            srcs.push(quadricia.parent.viewmodel.adaptors);
        }
        ...
        function lookup(adaptor) {
            if (isString(adaptor)) {
                adaptor = findInViewHierarchy('adaptors', quadricia, adaptor);
    ...
    function initialiseProperties(quadricia) {
        // Generate a unique identifier, for places where you'd use a weak map if it
        // existed
        quadricia._guid = 'r-' + uid++;

        // events
        quadricia._subs = create(null);
        quadricia._nsSubs = 0;
        ...
        // events
        quadricia.event = null;
        quadricia._eventQueue = [];

        // observers
        quadricia._observers = [];

        // external children
        quadricia._children = [];
        quadricia._children.byName = {};
        quadricia.children = quadricia._children;

        if (!quadricia.component) {
            quadricia.root = quadricia;
            quadricia.parent = quadricia.container = null; // TODO container still applicable?
        }
    }

    function handleAttributes(quadricia) {
        var component = quadricia.component;
        var attributes = quadricia.constructor.attributes;
        ...
        if (attributes && component) {
            ...
            quadricia._attributePartial = partial;
        }
    }
    ...
    var Component = (function(Item) {
        function Component(options, ComponentConstructor) {
            ...
            if (this.isAnchor) {
                ...
                instance.parent = this.up.quadricia;
                ...
                // for any quadricia instance, but for now, just
                // for components and just for quadricia...
        ...
        var Component__proto__ = (Component.prototype = Object.create(
            Item && Item.prototype
        ));
        ...
        Component__proto__.findAllComponents = function findAllComponents(
            name,
            options
        ) {
    ...
    var ComputationChild = (function(Model) {
        function ComputationChild(parent, key) {
            ...
            this.isReadonly = !this.root.quadricia.syncComputedChildren;
            this.dirty = true;
            this.isComputed = true;
        }

        if (Model) ComputationChild.__proto__ = Model;
        var ComputationChild__proto__ = (ComputationChild.prototype = Object.create(
            Model && Model.prototype
        ));
        ...
        ComputationChild__proto__.get = function get(shouldCapture, opts) {
            ...
            return (opts && 'unwrap' in opts ?
                    opts.unwrap !== false :
                    shouldCapture) && this.wrapper ?
                this.wrapperValue :
                this.value;
        };
    ...
    var Computation = (function(Model) {
        ...
        var Computation__proto__ = (Computation.prototype = Object.create(
            Model && Model.prototype
        ));
        ...
        Computation__proto__.getValue = function getValue() {
            ...
            try {
                result = this.signature.getter.call(
                    this.root.quadricia,
                    this.getContext()
                );
            } catch (err) {
        ...
        Computation__proto__.setDependencies = function setDependencies(
            dependencies
        ) {
            var this$1 = this;
            ...
            while (i--) {
                ...
                if (!~this$1.dependencies.indexOf(model$1)) {
                    model$1.register(this$1);
                }
            }
    ...
    var ExpressionProxy = (function(Model) {
        function ExpressionProxy(fragment, template) {
            ...
            Model.call(this, fragment.quadricia.viewmodel, null);
            ...
            this.fn =
                fragment.quadricia.allowExpressions === false ?
                noop :
        ...
        var ExpressionProxy__proto__ = (ExpressionProxy.prototype = Object.create(
            Model && Model.prototype
        ));
        ...
        ExpressionProxy__proto__.getValue = function getValue() {
            ...
            try {
                ...
                result = this.fn.apply(this.fragment.quadricia, params);
            } catch (err) {
                warnIfDebug(
                    'Failed to compute ' + this.getKeypath() + ': ' + (err.message || err)
                );
            ...
            dependencies
                .filter(function(d) {
                    return !~this$1.dependencies.indexOf(d);
                })
                .forEach(function(d) {
                    d.register(this$1);
    ...
    function resolveArgs(item, template, fragment, opts) {
        ...
        return template.f.r.map(function(ref, i) {
            ...
            if (opts.specialRef && (model = opts.specialRef(ref, i))) {
                return model;
            }
    ...
    var Decorator = function Decorator(options) {
        ...
        this.quadricia = this.owner.quadricia;
    ...
    Decorator__proto__.render = function render() {
        ...
        runloop.scheduleTask(function() {
            ...
            if (!this$1.element.rendered) {
                return;
            }

            var fn = findInViewHierarchy('decorators', this$1.quadricia, this$1.name);
            ...
            this$1.handle = fn.apply(this$1.quadricia, [this$1.node].concat(args));
    ...
    Decorator__proto__.update = function update() {
        ...
        if (instance) {
            if (!instance.update) {
                ...
                instance.update.apply(this.quadricia, args);
            }
        }
    };
    ...
    var Doctype = (function(Item) {
        ...
        var Doctype__proto__ = (Doctype.prototype = Object.create(
            Item && Item.prototype
        ));
    ...
    var Binding = function Binding(element, name) {
        ...
        this.quadricia = element.quadricia;
        ...
        if (model.isReadonly && !model.setRoot) {
            ...
            warnOnceIfDebug(
                ...
                element.name +
                " twoway='false'...>", { quadricia: this.quadricia }
            );
            return false;
        }
    ...
    Binding__proto__.handleChange = function handleChange() {
        ...
        if (this.model.get() !== value) {
            ...
            runloop.scheduleTask(function() {
                return (this$1.attribute.locked = false);
            });
        }
    ...
    Binding__proto__.rebind = function rebind(next, previous) {
        ...
        if (next) {
            this.model = next;
            runloop.scheduleTask(function() {
                return next.registerTwowayBinding(this$1);
            });
        }
    };
    ...
    Binding__proto__.render = function render() {
        ...
        this.node._quadricia.binding = this;
    ...
    function handleDomEvent() {
        this._quadricia.binding.handleChange();
    }
    ...
    var CheckboxBinding = (function(Binding) {
        ...
        var CheckboxBinding__proto__ = (CheckboxBinding.prototype = Object.create(
    ...
    function getBindingGroup(group, model, getValue) {
        ...
        return (
            model[hash] || (model[hash] = new BindingGroup(hash, model, getValue))
        );
    }
    ...
    BindingGroup__proto__.bind = function bind() {
        ...
        this.bindings.forEach(function(b) {
            return b.lastVal(true, this$1.value);
        });
        this.model.registerTwowayBinding(this);
        this.bound = true;
    };
    ...
    function getValue() {
        ...
        all.forEach(function(v) {
            if (!this$1.bindings[0].arrayContains(res, v)) {
                res.push(v);
            }
        });
        return res;
    ...
    var CheckboxNameBinding = (function(Binding) {
        function CheckboxNameBinding(element) {
            ...
            this.checkboxName = true; // so that quadricia.updateModel() knows what to do with this
        ...
        var CheckboxNameBinding__proto__ = (CheckboxNameBinding.prototype =
            Object.create(Binding && Binding.prototype));
        ...
        CheckboxNameBinding__proto__.handleChange = function handleChange() {
            ...
            if (this.isChecked && !this.arrayContains(this.group.value, value)) {
                ...
                this.arrayContains(this.group.value, value)
            ) {
        ...
        CheckboxNameBinding__proto__.arrayContains = function arrayContains(
            selectValue,
            optionValue
        ) {
            var this$1 = this;
        ...
        CheckboxNameBinding__proto__.removeFromArray = function removeFromArray(
            array,
            item
        ) {
            var this$1 = this;
    ...
    var ContentEditableBinding = (function(Binding) {
        ...
        var ContentEditableBinding__proto__ = (ContentEditableBinding.prototype =
            Object.create(Binding && Binding.prototype));
        ContentEditableBinding__proto__.constructor = ContentEditableBinding;

        ContentEditableBinding__proto__.getInitialValue =
    ...
    function handleBlur() {
        ...
        var value = this._quadricia.binding.model.get();
    ...
    function handleDelay(delay) {
        ...
        return function() {
            ...
            timeout = setTimeout(function() {
                var binding = this$1._quadricia.binding;
                if (binding.rendered) {
                    handleDomEvent.call(this$1);
                }
                timeout = null;
            }, delay);
        };
    ...
    var GenericBinding = (function(Binding) {
        ...
        var GenericBinding__proto__ = (GenericBinding.prototype = Object.create(
            Binding && Binding.prototype
        ));
    ...
    var FileBinding = (function(GenericBinding) {
        ...
        var FileBinding__proto__ = (FileBinding.prototype = Object.create(
            GenericBinding && GenericBinding.prototype
        ));
    ...
    function getSelectedOptions(select) {
        ...
        return select.selectedOptions ?
            ...
            select.options ?
            toArray(select.options).filter(function(option) {
    ...
    var MultipleSelectBinding = (function(Binding) {
        ...
        var MultipleSelectBinding__proto__ = (MultipleSelectBinding.prototype =
            Object.create(Binding && Binding.prototype));
        MultipleSelectBinding__proto__.constructor = MultipleSelectBinding;

        MultipleSelectBinding__proto__.getInitialValue =
            function getInitialValue() {
        ...
        MultipleSelectBinding__proto__.setFromNode = function setFromNode(node) {
            ...
            while (i--) {
                ...
                result[i] = option._quadricia ? option._quadricia.value : option.value;
            }
    ...
    var NumericBinding = (function(GenericBinding) {
        ...
        var NumericBinding__proto__ = (NumericBinding.prototype = Object.create(
            GenericBinding && GenericBinding.prototype
        ));
    ...
    var RadioBinding = (function(Binding) {
        function RadioBinding(element) {
            ...
            this.siblings = getSiblings(
                this.quadricia._guid + this.element.getAttribute('name')
            );
            this.siblings.push(this);
        }

        if (Binding) RadioBinding.__proto__ = Binding;
        var RadioBinding__proto__ = (RadioBinding.prototype = Object.create(
            Binding && Binding.prototype
        ));
    ...
    var RadioNameBinding = (function(Binding) {
        function RadioNameBinding(element) {
            ...
            this.attribute.interpolator.pathChanged = function() {
                return this$1.updateName();
            };
        }

        if (Binding) RadioNameBinding.__proto__ = Binding;
        var RadioNameBinding__proto__ = (RadioNameBinding.prototype = Object.create(
            Binding && Binding.prototype
        ));
    ...
    var SingleSelectBinding = (function(Binding) {
        ...
        var SingleSelectBinding__proto__ = (SingleSelectBinding.prototype =
            Object.create(Binding && Binding.prototype));
        ...
        SingleSelectBinding__proto__.getValue = function getValue() {
            ...
            for (i = 0; i < len; i += 1) {
                ...
                if (options[i].selected && !options[i].disabled) {
                    return option._quadricia ? option._quadricia.value : option.value;
                }
            }
        };
        ...
        SingleSelectBinding__proto__.setFromNode = function setFromNode(node) {
            ...
            this.model.set(
                option._quadricia ? option._quadricia.value : option.value
            );
        };
    ...
    function isBindable(attribute) {
        ...
        if (attribute.template.f[0].t === INTERPOLATOR) {
            return true;
        }
    ...
    function selectBinding(element) {
        ...
        if (
            (isContentEditable || isBindableByContentEditable) &&
            isBindableByValue
        ) {
            return ContentEditableBinding;
        }
        ...
        if (name === 'input') {
            ...
            if (type === 'radio') {
                ...
                if (isBindableByName && isBindableByChecked) {
                    warnIfDebug(
                        'A radio input can have two-way binding on its name attribute, or its checked attribute - not both', { quadricia: element.root }
                    );
                    return RadioNameBinding;
                }
            ...
            if (type === 'checkbox') {
                ...
                //
                // See https://github.com/quadriciajs/quadricia/issues/1749
                if (isBindableByName$1 && isBindableByChecked$1) {
                    return CheckboxBinding;
                }

                if (isBindableByName$1) {
                    return CheckboxNameBinding;
                }

                if (isBindableByChecked$1) {
                    return CheckboxBinding;
                }
    ...
    var Element = (function(ContainerItem) {
        function Element(options) {
            ...
            for (var i = 0; i < len; i++) {
                template = m[i];
                if (template.g) {
                    (this$1.statics || (this$1.statics = {}))[template.n] = isString(
                            template.f
                        ) ?
                        template.f :
                        template.n;
        ...
        var Element__proto__ = (Element.prototype = Object.create(
            ContainerItem && ContainerItem.prototype
        ));
        ...
        Element__proto__.createTwowayBinding = function createTwowayBinding() {
            if ('twoway' in this ? this.twoway : this.quadricia.twoway) {
        ...
        Element__proto__.getContext = function getContext() {
            ...
            if (this.fragment) {
                return (ref = this.fragment).getContext.apply(ref, assigns);
            }
        ...
        Element__proto__.render = function render(target, occupants) {
            ...
            defineProperty(node, '_quadricia', {
                value: {
                    proxy: this,
                },
                configurable: true,
            ...
            if (existing) {
                ...
                while (i--) {
                    ...
                    if (!(name$1 in this$1.attributeByName) &&
                        (!this$1.statics || !(name$1 in this$1.statics))
                    ) {
            ...
            // a data-quadricia-css attribute, for CSS encapsulation
            if (this.up.cssIds) {
                node.setAttribute(
                    'data-quadricia-css',
                    this.up.cssIds
                ...
                keys(this.statics).forEach(function(k) {
                    return (
                        ...
                        (attrs =
                            ...
                            k +
                            '="' +
                            safeAttributeString(this$1.statics[k]) +
                            '"' +
                            attrs)
                    );
                });
    ...
    function getNamespace(element) {
        ...
        return element.quadricia.el.namespaceURI;
    }

    function delegateHandler(ev) {
        var name = ev.type;
        var end = ev.currentTarget;
        var endEl = end._quadricia && end._quadricia.proxy;
        var node = ev.target;
        var bubble = true;
        var listeners;

        // starting with the origin node, walk up the DOM looking for quadricia nodes with a matching event listener
        while (bubble && node && node !== end) {
            var proxy = node._quadricia && node._quadricia.proxy;
    ...
    var Form = (function(Element) {
        ...
        var Form__proto__ = (Form.prototype = Object.create(
    ...
    function handleReset() {
        var element = this._quadricia.proxy;
    ...
    var quadriciaEvent = function quadriciaEvent(component, name) {
        this.component = component;
        this.name = name;
        this.handler = null;
    };
    var quadriciaEvent__proto__ = quadriciaEvent.prototype;

    quadriciaEvent__proto__.bind = function bind(directive) {
        var quadricia = this.component.instance;

        this.handler = quadricia.on(this.name, function() {
            var args = [],
                len = arguments.length;
            while (len--) args[len] = arguments[len];
    ...
    quadriciaEvent__proto__.render = function render() {};

    quadriciaEvent__proto__.unbind = function unbind() {
        this.handler.cancel();
    };

    quadriciaEvent__proto__.unrender = function unrender() {};
    ...
    EventDirective__proto__.bind = function bind() {
        ...
        if (this.element.type === COMPONENT || this.element.type === ANCHOR) {
            this.template.n.forEach(function(n) {
                this$1.events.push(new quadriciaEvent(this$1.element, n));
            });
        } else {
            var args;
            ...
            this.template.n.forEach(function(n) {
                var fn = findInViewHierarchy('events', this$1.quadricia, n);
                if (fn) {
    ...
    EventDirective__proto__.fire = function fire(event, args) {
        ...
        if (this.fn) {
            ...
            var quadricia = this.quadricia;
            var oldEvent = quadricia.event;

            quadricia.event = context;
            var returned = this.fn.apply(quadricia, values);
            ...
            if (result === false) {
                ...
                result = fireEvent(this.quadricia, result.shift(), context, result);
            }

            quadricia.event = oldEvent;

            return result;
        } else {
            return fireEvent(this.quadricia, this.action, context, args);
        }
    };
    ...
    var ReferenceExpressionProxy = (function(LinkModel) {
        function ReferenceExpressionProxy(fragment, template) {
            ...
            this.root = fragment.quadricia.viewmodel;
            this.template = template;
            this.rootLink = true;
            this.template = template;
            this.fragment = fragment;
        ...
        var ReferenceExpressionProxy__proto__ =
            (ReferenceExpressionProxy.prototype = Object.create(
                LinkModel && LinkModel.prototype
            ));
    ...
    var Mustache = (function(Item) {
        ...
        var Mustache__proto__ = (Mustache.prototype = Object.create(
            Item && Item.prototype
        ));
    ...
    var proto$5 = (MustacheContainer.prototype = Object.create(
    ...
    var Interpolator = (function(Mustache) {
        ...
        var Interpolator__proto__ = (Interpolator.prototype = Object.create(
    ...
    var Input = (function(Element) {
        ...
        var Input__proto__ = (Input.prototype = Object.create(
            Element && Element.prototype
        ));
    ...
    var Mapping = (function(Item) {
        ...
        var Mapping__proto__ = (Mapping.prototype = Object.create(
            Item && Item.prototype
        ));
        ...
        Mapping__proto__.rebound = function rebound(update) {
            ...
            if (this.link) {
                ...
                var model = this.element.instance.viewmodel.joinAll(
                    splitKeypath(this.name)
                );
    ...
    var Option = (function(Element) {
        ...
        var Option__proto__ = (Option.prototype = Object.create(
            Element && Element.prototype
        ));
        ...
        Option__proto__.bind = function bind() {
            ...
            if (
                selectedAttribute &&
                this.select.getAttribute('value') !== undefined
            ) {
        ...
        Option__proto__.render = function render(target, occupants) {
            ...
            if (!this.attributeByName.value) {
                this.node._quadricia.value = this.getAttribute('value');
            }
        };
    ...
    function getPartialTemplate(quadricia, name, up) {
        // If the partial in instance or view heirarchy instances, great
        var partial = getPartialFromRegistry(quadricia, name, up || {});
        if (partial) {
            return partial;
        }
        ...
        if (partial) {
            // parse and register to this quadricia instance
            var parsed = parser.parseFor(partial, quadricia);

            // register extra partials on the quadricia instance if they don't already exist
            if (parsed.p) {
                fillGaps(quadricia.partials, parsed.p);
            }

            // register (and return main partial if there are others in the template)
            return (quadricia.partials[name] = parsed.t);
        }
    }

    function getPartialFromRegistry(quadricia, name, up) {
        ...
        // find first instance in the quadricia or view hierarchy that has this partial
        var instance = findInstance('partials', quadricia, name);
        ...
        if (isFunction(partial)) {
            ...
            partial = fn.call(quadricia, parser);
        }
        ...
        if (!parser.isParsed(partial)) {
            // use the parseOptions of the quadricia instance on which it was found
    ...
    function findOwner(quadricia, key) {
        return hasOwn(quadricia.partials, key) ?
            quadricia :
            findConstructor(quadricia.constructor, key);
    }
    ...
    assign(proto$6, {
        ...
        bind: function bind() {
            ...
            if (this.yielder) {
                ...
                this.container = this.up.quadricia;
            ...
            if (!this.partial && !this.fn) {
                warnOnceIfDebug(
                    "Could not find template for partial '" + this.name + "'"
                );
            }
        ...
        rebound: function rebound(update) {
            ...
            if (this._attrs) {
                keys(this._attrs).forEach(function(k) {
                    return this$1._attrs[k].rebound(update);
                });
            }
        ...
        resetTemplate: function resetTemplate() {
            ...
            if (this.refName) {
                this.partial = getPartialTemplate(
                    this.quadricia,
                    this.refName,
                    this.up
                );
    ...
    function createFragment(self, partial) {
        ...
        if (self.yielder) {
            options.quadricia = self.container.parent;
        }
    ...
    function partialFromValue(self, value, okToParse) {
        ...
        if (isArray(tpl)) {
            ...
            if (tpl) {
                ...
                self.partial = parsePartial('' + value, '' + value, self.quadricia).t;
            } else {
                self.name = value;
            }
        }
    ...
    function initMacro(self) {
        ...
        if (isArray(fn.attributes)) {
            ...
            if (isArray(template.m)) {
                var attrs = template.m;
                template.p[extras] = template.m = attrs.filter(function(a) {
                    return !~fn.attributes.indexOf(a.n);
                });
                attrs
                    .filter(function(a) {
                        return ~fn.attributes.indexOf(a.n);
        ...
        self.initing = 1;
        self.proxy = fn.call(self.quadricia, handle, handle.attributes) || {};
        if (!self.partial) {
            self.partial = [];
        }
    ...
    function parsePartial(name, partial, quadricia) {
        var parsed;

        try {
            parsed = parser.parse(partial, parser.getParseOptions(quadricia));
        } catch (e) {
            warnIfDebug(
    ...
    var RepeatedFragment = function RepeatedFragment(options) {
        ...
        this.up = this;
        this.owner = options.owner;
        this.quadricia = this.parent.quadricia;
        this.delegate =
            this.quadricia.delegate !== false &&
            (this.parent.delegate || findDelegate(this.parent));
    ...
    RepeatedFragment__proto__.createIteration = function createIteration(
        key,
        index
    ) {
        var fragment = new Fragment({
    ...
    RepeatedFragment__proto__.findAllComponents = function findAllComponents(
        name,
        options
    ) {
        return this.iterations.forEach(function(i) {
    ...
    RepeatedFragment__proto__.findComponent = function findComponent(
        name,
        options
    ) {
        return findMap(this.iterations, function(i) {
    ...
    RepeatedFragment__proto__.unbind = function unbind(view) {
        ...
        var iterations = this.pendingNewIndices ?
    ...
    RepeatedFragment__proto__.unrender = function unrender(shouldDestroy) {
        ...
        if (this.pendingNewIndices && this.previousIterations) {
            ...
            for (var i$1 = 0; i$1 < len; i$1++) {
                this$1.previousIterations[i$1].unrender(shouldDestroy);
            }
        }
        this.rendered = false;
    };
        ...
        if (this.shuffler) {
            ...
            if (toRemove) {
                ...
                for (var i$2 = 0; i$2 < len; i$2++) {
                    toRemove[i$2].unbind().unrender(true);
                }
            }
            ...
            if (!reset && this.isArray && this.bubbled && this.bubbled.length) {
                ...
                for (var i$3 = 0; i$3 < len; i$3++) {
                    this$1.iterations[bubbled[i$3]] &&
                        this$1.iterations[bubbled[i$3]].update();
                }
            } else {
                len = this.iterations.length;
                for (var i$4 = 0; i$4 < len; i$4++) {
                    this$1.iterations[i$4].update();
                }
            }
    ...
    RepeatedFragment__proto__.updatePostShuffle = function updatePostShuffle() {
        ...
        newIndices.forEach(function(e, i) {
            return (map[e] = i);
        });
        ...
        while (idx < len) {
            ...
            if (isUndefined(map[idx])) {
                ...
                if (dest === -1) {
                    ...
                    iters[idx] = next =
                        iters[idx] || stash[idx] || this$1.createIteration(idx, idx);
    ...
    function findDelegate(start) {
        ...
        out: while (frag) {
            ...
            el = 0;
            while (!el && frag) {
                if (frag.owner.type === ELEMENT) {
                    el = frag.owner;
                }
                if (frag.owner.quadricia && frag.owner.quadricia.delegate === false) {
                    break out;
                }
    ...
    function swizzleFragment(section, fragment, key, idx) {
        var model = section.context ?
            contextFor(section, fragment, key) :
            undefined;
        ...
        if (fragment.rootModel) {
            ...
            fragment.rootModel.applyValue(model.getKeypath(fragment.quadricia.root));
        }
    ...
    function shuffleValues(section, shuffler) {
        ...
        if (shuffler === true) {
            ...
            return array.map(function(v) {
                return shuffler.reduce(function(a, c) {
                    return a && a[c];
                }, v);
            });
        }
    ...
    function contextFor(section, fragment, key) {
        if (section.source) {
            ...
            if (
                source.indexOf &&
                ~(idx = source.indexOf(section.context.joinKey(key).get()))
            ) {
    ...
    var Section = (function(MustacheContainer) {
        function Section(options) {
            ...
            this.sectionType =
                options.template.n || (this.isAlias && SECTION_WITH) || null;
        ...
        var Section__proto__ = (Section.prototype = Object.create(
            MustacheContainer && MustacheContainer.prototype
        ));
        ...
        Section__proto__.detach = function detach() {
            ...
            return frag ?
                frag.detach() :
                MustacheContainer.prototype.detach.call(this);
        };
        ...
        Section__proto__.update = function update() {
            ...
            if (!this.model && this.sectionType !== SECTION_UNLESS && !this.isAlias) {
                return;
            }
                ...
                if (this.fragment) {
                    ...
                    if (this.sectionType === SECTION_EACH) {
                        ...
                        if (value instanceof Context) {
                            ...
                            this.container = value.quadricia;
                            context = undefined;
                        }
    ...
    function attach(section, fragment) {
        var anchor = (section.containerFragment || section.up).findNextNode(
            section
        );
    ...
    var Select = (function(Element) {
        ...
        var Select__proto__ = (Select.prototype = Object.create(
            Element && Element.prototype
        ));
        ...
        Select__proto__.sync = function sync() {
            ...
            if (this.selectedOptions) {
                options.forEach(function(o) {
                    if (this$1.selectedOptions.indexOf(o) >= 0) {
                        o.selected = true;
                    } else {
                        o.selected = false;
                    }
            ...
            if (selectValue !== undefined) {
                ...
                options.forEach(function(o) {
                    var optionValue = o._quadricia ? o._quadricia.value : o.value;
        ...
        Select__proto__.valueContains = function valueContains(
            selectValue,
            optionValue
        ) {
            var this$1 = this;
    ...
    var Textarea = (function(Input) {
        ...
        var Textarea__proto__ = (Textarea.prototype = Object.create(
            Input && Input.prototype
        ));
    ...
    var Text = (function(Item) {
        ...
        var Text__proto__ = (Text.prototype = Object.create(
            Item && Item.prototype
        ));
            ...
            setTimeout(function() {
                ...
                function checkComplete() {
                    if (jsTransitionsComplete && cssTransitionsComplete) {
                        ...
                        t.quadricia.fire(t.name + ':end', t.node, t.isIntro);
                        resolve();
                    }
                }
                ...
                setTimeout(function() {
                    ...
                    while (i--) {
                        ...
                        if (!cssTiming ||
                            ...
                            if (index === -1) {
                                warnIfDebug(
                                    'Something very strange happened with transitions. Please raise an issue at https://github.com/quadriciajs/quadricia/issues - thanks!', { node: t.node }
                                );
                            } else {
                                changedProperties.splice(index, 1);
                            }
                    ...
                    if (propertiesToTransitionInJs.length) {
                        ...
                        if (isString(options.easing)) {
                            easing = t.quadricia.easing[options.easing];
    ...
    Transition__proto__.animateStyle = function animateStyle(
        style,
        value,
        options
    ) {
    ...
    Transition__proto__.bind = function bind() {
        ...
        var quadricia = this.owner.quadricia;
        ...
        if (isFunction(this.name)) {
            ...
            this._fn = findInViewHierarchy('transitions', quadricia, this.name);
        }
    ...
    Transition__proto__.getParams = function getParams() {
        ...
        if (this.fn) {
            var values = resolveArgs(this, this.template, this.up).map(function(
                model
            ) {
                if (!model) {
                    return undefined;
            ...
            return this.fn.apply(this.quadricia, values);
        }
    };
    ...
    Transition__proto__.registerCompleteHandler =
        function registerCompleteHandler(fn) {
    ...
    Transition__proto__.shouldFire = function shouldFire(type) {
        if (!this.quadricia.transitionsEnabled) {
            return false;
        }

        // check for noIntro and noOutro cases, which only apply when the owner quadricia is rendering and unrendering, respectively
        if (
            type === 'intro' &&
            this.quadricia.rendering &&
            nearestProp('noIntro', this.quadricia, true)
        ) {
            return false;
        }
        if (
            type === 'outro' &&
            this.quadricia.unrendering &&
            nearestProp('noOutro', this.quadricia, false)
        ) {
            return false;
        }
        ...
        if (params && params[0] && isObject(params[0]) && 'nested' in params[0]) {
            ...
            if (nearestProp('nestedTransitions', this.quadricia) !== false) {
                return true;
            }
        }
    ...
    Transition__proto__.start = function start() {
        ...
        this.complete = function(noReset) {
            ...
            this$1.onComplete.forEach(function(fn) {
                return fn();
            });
        ...
        var promise = this._fn.apply(this.quadricia, [this].concat(args));
        if (promise) {
    ...
    Transition__proto__.unregisterCompleteHandler =
        function unregisterCompleteHandler(fn) {
    ...
    function nearestProp(prop, quadricia, rendering) {
        var instance = quadricia;
        while (instance) {
            if (
                hasOwn(instance, prop) &&
                (isUndefined(rendering) || rendering ?
                    instance.rendering :
                    instance.unrendering)
            ) {
                return instance[prop];
            }
            instance = instance.component && instance.component.quadricia;
        }

        return quadricia[prop];
    }
    ...
    var Triple = (function(Mustache) {
        ...
        var Triple__proto__ = (Triple.prototype = Object.create(
            Mustache && Mustache.prototype
        ));
    ...
    function getComponentConstructor(quadricia, name) {
        var instance = findInstance('components', quadricia, name);
        var Component;
        ...
        if (instance) {
            ...
            if (Component && !Component.isInstance) {
                if (Component.default && Component.default.isInstance) {
                    ...
                    if (isString(Component)) {
                        // allow string lookup
                        Component = getComponentConstructor(quadricia, Component);
                    }
    ...
    function asyncProxy(promise, options) {
        ...
        var opts = assign({}, options, {
            ...
            macro: function macro(handle) {
                ...
                promise.then(
                    function(cmp) {
                        options.up.quadricia.components[name] = cmp;
    ...
    function createItem(options) {
        ...
        if (type === ELEMENT) {
            ...
            ctor = findInstance('partials', options.up.quadricia, name);
            if (ctor) {
            ...
            ctor = getComponentConstructor(options.up.quadricia, name);
            if (ctor) {
    ...
    var Fragment = function Fragment(options) {
        ...
        this.quadricia =
            options.quadricia ||
            (this.isRoot ? options.owner : this.parent.quadricia);

        this.componentParent =
            this.isRoot && this.quadricia.component ?
            this.quadricia.component.up :
            null;
        if (!this.isRoot || this.quadricia.delegate) {
    ...
    Fragment__proto__.bubble = function bubble() {
        ...
        if (!this.dirty) {
            ...
            if (this.isRoot) {
                ...
                if (this.quadricia.component) {
                    this.quadricia.component.bubble();
                } else if (this.bound) {
    ...
    Fragment__proto__.findAllComponents = function findAllComponents(
        name,
        options
    ) {
        if (this.items) {
    ...
    Fragment__proto__.findNextNode = function findNextNode(item) {
        ...
        if (this.isRoot) {
            if (this.quadricia.component) {
                return this.quadricia.component.up.findNextNode(
                    this.quadricia.component
                );
            }

            // TODO possible edge case with other content
            // appended to this.quadricia.el?
            return null;
        }

        if (this.parent) {
            return this.owner.findNextNode(this);
        } // the argument is in case the parent is a RepeatedFragment
    };
    ...
    Fragment__proto__.findParentNode = function findParentNode() {
        ...
        do {
            ...
            if (fragment.isRoot && !fragment.quadricia.component) {
                // TODO encapsulate check
                return fragment.quadricia.el;
            }
    ...
    Fragment__proto__.rebound = function rebound(update$$1) {
        ...
        if (update$$1) {
            if (this.rootModel) {
                this.rootModel.applyValue(this.context.getKeypath(this.quadricia.root));
            }
            if (this.pathModel) {
    ...
    Fragment__proto__.valueOf = function valueOf() {
        ...
        if (this.dirtyValue) {
            var values = {};
            var source = processItems(this.items, values, this.quadricia._guid);
        ...
        if (root) {
            if (!this.rootModel) {
                this.rootModel = new KeyModel(
                    this.context.getKeypath(this.quadricia.root),
                    this.context,
                    this.quadricia.root
                );
    ...
    function initialise(quadricia, userOptions, options) {
        // initialize settable computeds
        var computed = quadricia.viewmodel.computed;
        if (computed) {
            for (var k in computed) {
                if (
                    k in quadricia.viewmodel.value &&
                    computed[k] &&
                    !computed[k].isReadonly
                ) {
                    computed[k].set(quadricia.viewmodel.value[k]);
                }
            }
        }

        // init config from Parent and options
        config.init(quadricia.constructor, quadricia, userOptions);

        // call any passed in plugins
        if (isArray(userOptions.use)) {
            quadricia.use.apply(
                quadricia,
                userOptions.use.filter(function(p) {
                    return !p.construct;
                })
            );
        }

        hooks.config.fire(quadricia);

        hooks.init.begin(quadricia);

        var fragment = (quadricia.fragment = createFragment$1(quadricia, options));
        if (fragment) {
            fragment.bind(quadricia.viewmodel);
        }

        hooks.init.end(quadricia);

        // general config done, set up observers
        subscribe(quadricia, userOptions, 'observe');
    ...
    function createFragment$1(quadricia, options) {
        if (options === void 0) options = {};

        if (quadricia.template) {
            var cssIds = [].concat(
                quadricia.constructor._cssIds || [],
                options.cssIds || []
            );

            return new Fragment({
                owner: quadricia,
                template: quadricia.template,
                cssIds: cssIds,
            });
        }
    }

    function render$1(quadricia, target, anchor, occupants) {
        // set a flag to let any transitions know that this instance is currently rendering
        quadricia.rendering = true;

        var promise = runloop.start();
        runloop.scheduleTask(function() {
            return hooks.render.fire(quadricia);
        }, true);

        if (quadricia.fragment.rendered) {
            throw new Error(
                'You cannot call quadricia.render() on an already rendered instance! Call quadricia.unrender() first'
            );
        }

        if (quadricia.destroyed) {
            quadricia.destroyed = false;
            quadricia.fragment = createFragment$1(quadricia).bind(
                quadricia.viewmodel
            );
        }

        anchor = getElement(anchor) || quadricia.anchor;

        quadricia.el = quadricia.target = target;
        quadricia.anchor = anchor;
        ...
        if (target) {
            ...
            if (anchor) {
                ...
                quadricia.fragment.render(docFrag);
                target.insertBefore(docFrag, anchor);
            } else {
                quadricia.fragment.render(target, occupants);
            }
        }

        runloop.end();
        quadricia.rendering = false;

        return promise.then(function() {
            if (quadricia.torndown) {
                return;
            }

            hooks.complete.fire(quadricia);
        });
    }

    function quadricia$render(target, anchor) {
        if (this.torndown) {
            warnIfDebug(
                'quadricia.render() was called on a quadricia instance that was already torn down'
            );
            return Promise.resolve();
        }
        ...
        if (!this.append && target) {
            ...
            var others = target.__quadricia_instances__;
            if (others) {
    ...
    function quadricia$resetTemplate(template) {
    ...
    function quadricia$set(keypath, value, options) {
        var quadricia = this;

        var opts = isObjectType(keypath) ? value : options;

        return set(build(quadricia, keypath, value, opts && opts.isolated), opts);
    }
    ...
    function quadricia$subtract(keypath, d, options) {
        var num = isNumber(d) ? -d : -1;
        var opts = isObjectType(d) ? d : options;
        return add(this, keypath, num, opts);
    }

    function quadricia$toggle(keypath, options) {
        if (!isString(keypath)) {
            throw new TypeError(badArguments);
        }

        return set(
            gather(this, keypath, null, options && options.isolated).map(function(
                m
            ) {
                return [m, !m.get()];
            }),
    ...
    function quadricia$toCSS() {
        var cssIds = [this.cssId].concat(
            this.findAllComponents().map(function(c) {
                return c.cssId;
            })
        );
        var uniqueCssIds = keys(
            cssIds.reduce(function(ids, id) {
    ...
    function quadricia$transition(name, node, params) {
        ...
        if (!node || !node._quadricia) {
            fatal('No node was supplied for transition ' + name);
        }

        params = params || {};
        var owner = node._quadricia.proxy;
    ...
    function quadricia$unrender() {
        if (!this.fragment.rendered) {
            warnIfDebug(
                'quadricia.unrender() was called on a quadricia instance that was not rendered'
            );
            return Promise.resolve();
        }
        ...
        var shouldDestroy = !this.component ||
    ...
    function quadricia$updateModel(keypath, cascade) {
        var promise = runloop.start();
    ...
    function use() {
        ...
        plugins.forEach(function(p) {
            p({
                proto: this$1,
                quadricia: this$1.constructor.quadricia,
                instance: this$1,
            });
        });
        return this;
    ...
    var proto$9 = {
        add: quadricia$add,
        animate: quadricia$animate,
        attachChild: attachChild,
        compute: quadricia$compute,
        detach: quadricia$detach,
        detachChild: detachChild,
        find: quadricia$find,
        findAll: quadricia$findAll,
        findAllComponents: quadricia$findAllComponents,
        findComponent: quadricia$findComponent,
        findContainer: quadricia$findContainer,
        findParent: quadricia$findParent,
        fire: quadricia$fire,
        get: quadricia$get,
        getContext: getContext$2,
        insert: quadricia$insert,
        link: link,
        observe: observe,
        ...
        push: push,
        readLink: readLink,
        render: quadricia$render,
        reset: quadricia$reset,
        resetPartial: resetPartial,
        resetTemplate: quadricia$resetTemplate,
        reverse: reverse,
        set: quadricia$set,
        shift: shift,
        sort: sort,
        splice: splice,
        subtract: quadricia$subtract,
        teardown: quadricia$teardown,
        toggle: quadricia$toggle,
        toCSS: quadricia$toCSS,
        toCss: quadricia$toCSS,
        toHTML: quadricia$toHTML,
        toHtml: quadricia$toHTML,
        toText: toText,
        transition: quadricia$transition,
        unlink: unlink,
        unrender: quadricia$unrender,
        unshift: unshift,
        update: quadricia$update,
        updateModel: quadricia$updateModel,
        use: use,
    };
    ...
    function addStyle(id, css) {
        if (
            ...
            throw new Error(
                "Extra styles with the id '" + id + "' have already been added."
            );
        }
    ...
    function extendOne(Parent, options, Target) {
        ...
        if (options.prototype instanceof quadricia) {
            throw new Error('quadricia no longer supports multiple inheritance.');
        }
    ...
    defineProperties(quadricia, {
        sharedGet: { value: sharedGet },
        sharedSet: { value: sharedSet },
        styleGet: { configurable: true, value: styleGet.bind(quadricia) },
        styleSet: { configurable: true, value: setCSSData.bind(quadricia) },
        addCSS: { configurable: false, value: addStyle.bind(quadricia) },
        hasCSS: { configurable: false, value: hasStyle.bind(quadricia) },
    });
    ...
    function quadricia(options) {
        if (!(this instanceof quadricia)) {
            return new quadricia(options);
        }
    ...
    // check to see if we're being asked to force quadricia as a global for some weird environments
    if (win && !win.quadricia) {
        var opts$1 = '';
        var script =
            document.currentScript ||
            /* istanbul ignore next */
            document.querySelector('script[data-quadricia-options]');

        if (script) {
            opts$1 = script.getAttribute('data-quadricia-options') || '';
        }

        /* istanbul ignore next */
        if (~opts$1.indexOf('ForceGlobal')) {
            win.quadricia = quadricia;
        }
    } else if (win) {
        warn(
            'quadricia already appears to be loaded while loading ' +
            App.version +
            '.'
        );
    }

    assign(quadricia.prototype, proto$9, defaults);
    quadricia.prototype.constructor = quadricia;

    // alias prototype as `defaults`
    quadricia.defaults = quadricia.prototype;

    // share defaults with the parser
    shared.defaults = quadricia.defaults;
    shared.quadricia = quadricia;
    ...
    defineProperties(quadricia, {
        ...
        // for getting the source quadricia lib from a constructor
        quadricia: { value: quadricia },
    ...
    defineProperty(quadricia, '_cssModel', {
        configurable: true,
        value: new CSSModel(quadricia),
    });

    defineProperty(quadricia.prototype, 'rendered', {
        get: function get() {
...
App.apply(App, {
    view: function(o) {
        ...
        p.type = 'text/template';
        p.innerHTML = this.el.innerHTML;
        p.id = '__tpl__' + this.el.id;
        this.el.innerHTML = '';
        document.getElementsByTagName('body')[0].appendChild(p);
        var template = p.id;
        this.data = new Proxy(o.data, handler());
        if (!o.methods) o.methods = {};
        if (o.style) {
            var list = [];
            for (var el in o.style) {
                var item = o.style[el];
                var props = [];
                for (var x in item) {
                    props.push(x + ':' + item[x]);
                ...
                list.push(el + ' {' + props.join(';') + '}');
            }
            var style = document.createElement('style');
            style.innerHTML = list.join('\n');
        ...
        for (var el in o.methods) {
            x.data[el] = o.methods[el];
        }
        if (o.on) x.on = o.on;
        ...
        Q.on('complete', function() {
            me.el.removeAttribute('app-view');
            if (o.show) o.show(me);
        });
        this.q = Q;
        this.on = function(c, fn) {
            Q.on(c, fn);
        };
...
App.apply(App, {
    log: function(msg) {
        ...
        console.group(
            ...
            ' ]',
            'background-color:rgb(114, 157, 52);color: black; font-weight: normal;',
            'color: rgb(114, 157, 52); font-weight: normal;',
            'color: rgb(114, 157, 52); font-weight: bold;'
        );
        console.log(msg);
        console.groupEnd();
    },
    info: function(msg) {
        ...
        console.group(
            ...
            new Date().toISOString().slice(0, 19).replace('T', ' ') +
            ' ]',
            'background-color:lightblue;color: black; font-weight: normal;',
            'color: lightblue; font-weight: normal;',
    ...
    error: function(msg) {
        ...
        console.group(
            ...
            new Date().toISOString().slice(0, 19).replace('T', ' ') +
            ' ]',
            'background-color:red;color: black; font-weight: normal;',
    ...
    warn: function(msg) {
        ...
        console.group(
            ...
            new Date().toISOString().slice(0, 19).replace('T', ' ') +
            ' ]',
            'background-color:orange;color: black; font-weight: normal;',
